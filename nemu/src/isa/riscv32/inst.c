...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (src1 == src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bneq   , B, s->dnpc = (src1 != src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = ((int32_t)src1 <(int32_t)src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int32_t)src1 >(int32_t)src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = (src1 < src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = (src1 > src2)?s->pc + imm : s->dnpc; check_al(s->dnpc));
